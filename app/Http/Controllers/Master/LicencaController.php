...
class LicencaController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $indicadores = [
            ...
            'ativas' => Licenca::query()
                ->whereIn('status', [
                    Licenca::STATUS_ATIVA,
                    Licenca::STATUS_TESTE,
                ])
                ->where(function ($q) {
                    $q->whereNull('termina_em')
            ...
            'bloqueadas' => Licenca::query()
                ->whereIn('status', [
                    Licenca::STATUS_SUSPENSA,
                    Licenca::STATUS_CANCELADA,
                    Licenca::STATUS_VENCIDA,
                ])
                ->count(),
        ];

        return view(
            'master.licencas.index',
            compact('licencas', 'planos', 'indicadores')
        );
    }
    ...
    public function store(
        LicencaRequest $request,
        LicencaTokenService $tokenService
    ): RedirectResponse {
        [$licenca, $token] = DB::transaction(
            function () use ($request, $tokenService) {
                ...
                $this->historico(
                    $licenca,
                    'criada',
                    'Licença criada e token inicial gerado.',
                    $data
                );
        ...
        return redirect()
            ->route('master.licencas.edit', $licenca)
    ...
    public function edit(Licenca $licenca): View
    {
        ...
        $empresas = Empresa::query()
            ->where(function ($query) use ($licenca) {
                $query
                    ->whereDoesntHave('licenca')
                    ->orWhere('id', $licenca->empresa_id);
            })
            ->orderBy('nome_fantasia')
            ->get();
    ...
    public function renovar(
        ...
        $this->historico(
            $licenca,
            'renovada',
            "Licença renovada por {$request->integer('meses')} mês(es).",
            [
                'termina_em' => $licenca->termina_em,
            ]
        );
    ...
    public function regenerarToken(
        ...
        $this->historico(
            $licenca,
            'token_rotacionado',
            'Token do PontoWeb Agent regenerado.'
        );

        return back()
            ->with(
                'success',
                'Novo token gerado. Copie-o agora; ele não será exibido novamente.'
            )
            ->with('agent_token', $token);
    }
    ...
    private function aplicarDatasDeStatus(
        array &$data,
