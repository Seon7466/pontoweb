@php
    $navLink =
        'pw-focus group relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition duration-200';
    $navActive = '
bg-blue-500/15
text-blue-300
border-l-4
...
<aside
    class="fixed inset-y-0 left-0 z-50 flex w-64 -translate-x-full flex-col overflow-hidden border-r border-white/10 bg-slate-950 text-white shadow-2xl shadow-slate-950/30 transition-all duration-300 ease-out lg:translate-x-0"
    :class="{
    ...
    <div class="flex h-16 shrink-0 items-center justify-between border-b border-slate-800 px-4">
        <a href="{{ route('dashboard') }}"
            ...
            <span
                class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-600 text-sm font-bold text-white ring-1 ring-blue-500/30">
                PW
            </span>

            <span x-show="! sidebarCollapsed" x-transition.opacity class="min-w-0">
                <span class="block truncate text-base font-extrabold tracking-tight">PontoWeb</span>
    ...
    <nav class="pw-scrollbar flex-1 space-y-6 overflow-y-auto px-3 py-5">
        <div class="space-y-2">
            <a href="{{ route('dashboard') }}"
        ...
        <div>
            <p x-show="! sidebarCollapsed" x-transition.opacity
                class="mb-2 px-3 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">
                Cadastros
            </p>
