<x-app-layout>
    ...
    <x-pw.page-header
        title="Empresa"
        description="Consulte e atualize os dados da empresa vinculada à sua conta."
    >
        <x-slot name="actions">
    ...
    <x-pw.table-card
        ...
        description="Informações cadastrais utilizadas em toda a plataforma."
    >
        <x-slot name="actions">
            ...
            <tbody>
                @forelse ($items as $empresa)
                    <tr>
                        ...
                        <td class="whitespace-nowrap">{{ $empresa->cnpj }}</td>
                        <td>
                            <div>{{ $empresa->email ?: 'E-mail não informado' }}</div>
                            <div class="mt-0.5 text-xs text-slate-500">
                                {{ $empresa->telefone ?: 'Telefone não informado' }}
                            </div>
                        </td>
                        <td>
                            {{ collect([$empresa->cidade, $empresa->estado])->filter()->join(' / ') ?: 'Não informada' }}
                        </td>
                        <td>
